...
use iovariablesmod,  only : cfile_spinup,cfile_transient,soilfile,                            &
                            ...
                            lucc,cellindex,lonvect,latvect,co2vect,nolanduse,nclimv,calcforagers
...
write(0,'(a,2i5,2f8.2)')   'starting at:       ',srtx,srty,lonvect(srtx),latvect(srty)
...
do y = 1,cnty
  ...
  do x = 1,cntx
    ...
    in_master(idx)%cellarea = area(latvect(b),[30.,30.])
...
do i = 1,ncells
  call initstatevars(in_master(i),sv_master(i))
end do
...
!-------
!open the population presence file
