...
subroutine calcgpp(present,co2,soilpar,pftpar,fpc_grid,mdayl,mtemp,mpar_day,dphen_t,w,dpet,dprec,dmelt,   &
                   agpp,alresp,arunoff_surf,arunoff_drain,arunoff,mrunoff,dwscal365,dphen_w,dphen,wscal,mgpp,mlresp,  &
                   mw1,dw1,aaet,leafondays,leafoffdays,leafon,tree,raingreen,mat20,wscal_v)
...
real(sp), dimension(npft) :: lambdam = [ 0.9, 0.9, 0.9, 0.8, 0.8, 0.8, 0.9, 0.65, 0.4 ] ! optimal Ci/Ca ratio
    ...
    if (present(pft)) then
      ...
      if (gpd > 1.e-5) then  !canopy conductance
        ...
        do  !bisection root finding
          ...
          call photosynthesis(ca,mtemp(m),fpar,mpar_day(m),mdayl(m),c4(pft),xmid, &
