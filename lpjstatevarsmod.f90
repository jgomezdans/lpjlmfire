module lpjstatevarsmod

!NB this module is TEMPORARY and should be removed
...
type state_variables
  ...
  real(sp), dimension(ncvar) :: cpool_surf        !fast-decomposing soil C pool (gC/m2)
...
!
! !use iovariables, only : lu_turn_yrs
!
! implicit none
!
! integer :: i
...
real(sp) function area(lat,minutes)
  ...
  elevation = deg2rad * (lat + (resolution(2) / 2.0d0))

  deltalat = deg2rad * resolution(1)
  deltalon = deg2rad * resolution(2)

  cellarea = (2.0d0*radius**2*deltalon*cos(elevation)*sin((deltalat/2.0d0)))
