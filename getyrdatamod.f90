...
do i = 1,ncells
  ...
  if (calcforagers) then
    in_master(i)%human%presence = int(ibuf(x,y)%popd(1))
  else
    in_master(i)%human%presence = 0
  end if
...
use iovariablesmod, only : cfid,ibuf,timebuflen,climatemonths,srtx,cntx,srty,cnty,varinfo,soil,cellmask,input_sp,input_i2,nclimv
...
integer(i2), dimension(cntx,cnty) :: ivar
...
ncstat = nf90_inq_varid(popfid,'presence',varid)
if (ncstat /= nf90_noerr) call netcdf_err(ncstat)

ncstat = nf90_get_var(popfid,varid,ivar,start=[srtx,srty,srtt],count=[cntx,cnty,1])
if (ncstat /= nf90_noerr) call netcdf_err(ncstat)

ibuf%popd(1) = real(ivar)
...
use typesizes
use errormod,       only : netcdf_err,ncstat
use iovariablesmod, only : srtx,cntx,srty,cnty,topofid,elvid,slopeid,landfid,topotime,soil
